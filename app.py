...
def call_nebius_api(
    api_key: str, model: str, messages: list, max_tokens: int = 1000, temperature: float = 0.7
) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        response: requests.Response = requests.post(
            "https://api.studio.nebius.ai/v1/chat/completions", headers=headers, json=payload
        )
...
def welcome_screen():
    ...
    st.info("**Get started:** Enter your Nebius AI Studio API key in the sidebar to begin using the tools.", icon="👈")


def chatbot_interface():
    st.title("💬 AI Multi-Tool Chatbot")
    st.write("This chatbot uses Nebius AI Studio API. ")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                success, result = call_nebius_api(
                    api_key=api_key, model=model, messages=messages, max_tokens=max_tokens, temperature=temperature
                )
...
def text_checker_interface():
    ...
    input_text: str = st.text_area("Enter text to check:", height=300, placeholder="Paste your text here...")
...
def recipe_creator_interface():
    ...
    with col2:
        ...
        cooking_method: str = st.selectbox(
            "Preferred cooking method:", cooking_methods, help="Choose your preferred way of cooking"
        )

        st.subheader("⏱️ Options")
        servings: int = st.number_input("Number of servings:", min_value=1, max_value=12, value=4)
        dietary_restrictions: str = st.text_input("Dietary restrictions:", placeholder="vegetarian, gluten-free, etc.")
    ...
    if st.button("🍽️ Create Recipe", type="primary", disabled=not ingredients):
        with st.spinner("Creating your recipe..."):
            ...
            recipe_instruction: str = f"""Create a detailed recipe using the provided ingredients. Follow this exact format:
...
with st.sidebar:
    ...
    api_key = st.text_input(
        "Nebius AI Studio API Key", type="password", help="Enter your API key from Nebius AI Studio"
    )
    if not api_key:
    ...
    model = st.selectbox(
        "Select Model", options=list(model_options.keys()), index=0, help="Choose the AI model for your conversation"
    )
